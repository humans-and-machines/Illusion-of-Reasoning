...
```shell
sbatch --nodes=2 ../maxent-grpo/scripts/train.slurm --model Qwen2.5-1.5B-Instruct --task grpo --config demo --accelerator zero2 --dp 8 --tp 1
```
...
```shell
sbatch --job-name=cf-grpo --nodes=2 ../maxent-grpo/scripts/train.slurm --model Qwen2.5-Coder-7B-Instruct --task grpo --config codeforces --accelerator zero3 --dp 8 --tp 1
```

### Launching jobs on a Slurm cluster

If you have access to a Slurm cluster, we provide a `../maxent-grpo/scripts/train.slurm` script that will automatically queue training jobs for you. Here's how you can use it:

```shell
sbatch --job-name=open_r1 --nodes=1 ../maxent-grpo/scripts/train.slurm --model {model_name} --task {task} --config {config_suffix} --accelerator {accelerator}
```
...
```shell
sbatch --job-name=open_r1 --nodes=1 ../maxent-grpo/scripts/train.slurm --model OpenR1-Distill-7B --task sft --config distill --accelerator zero3
```
...
```shell
sbatch --job-name=open_r1 --nodes=2 ../maxent-grpo/scripts/train.slurm --model Qwen2.5-1.5B-Instruct --task grpo --config demo --accelerator zero2 --dp 4 --tp 2
```

> [!NOTE]
> The configuration in `../maxent-grpo/scripts/train.slurm` is optimised for the Hugging Face Compute Cluster and may require tweaking to be adapted to your own compute nodes.
