...
# ── Paths ───────────────────────────────────────────────────
SCRIPT_PATH="/n/fs/similarity/open-r1/carpark-inference.py"

# Tuned checkpoint root + step (override with env if needed)
MODEL_ROOT="${MODEL_ROOT:-/n/fs/similarity/open-r1/data/Qwen2.5-1.5B-Open-R1-GRPO-carpark-v1}"
CKPT_STEP="${CKPT_STEP:-750}"
CKPT_DIR="$MODEL_ROOT/checkpoint-$CKPT_STEP"

# Output
OUTPUT_ROOT="/n/fs/similarity/open-r1/results/GRPO-1.5B-carpark-compare-1shot"
mkdir -p "$OUTPUT_ROOT"
