...
from src.inference.domains.math import math_core as _math_core
...
:func:`src.inference.domains.math.math_core.run_inference_on_split`
    ...
    config = _math_core.MathInferenceConfig(
        split_name=split_name,
        output_dir=outdir,
        step=step,
        **config_defaults,
    )
    _math_core.run_inference_on_split(
        examples=examples,
        tokenizer=tokenizer,
        model=model,
        config=config,
...
def load_math500(
    ...
    """
    return _math_core.load_math500(cache_dir, split, seed, dataset_path)
