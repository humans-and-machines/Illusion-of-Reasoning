...
def _piece_dir(token: str) -> Tuple[str, str]:
    """
    Extract the piece identifier and direction from a canonical move token.

    :param token: Canonical token string such as ``\"Bv2\"``.
    :returns: Tuple ``(piece, direction)`` ignoring the step count.
    """
...
def _canon_join(tokens: List[str]) -> Optional[str]:
    """
    Canonicalize and join a list of move tokens.

    :param tokens: List of raw token strings (for example, ``[\"Bv2\", \"A>1\"]``).
    :returns: Comma-separated canonical string or ``None`` if any token is invalid.
    """
