...
from src.inference.utils._torch_stubs import (  # type: ignore[import-untyped]
    StoppingCriteriaStub,
    TorchStub,
)
...
def build_generate_kwargs(
    *,
    ...
    :param entropy_mode: Entropy computation mode (for example,
        ``\"reconsider\"`` or ``\"none\"``).
    :param temperature: Sampling temperature; when ``None`` or non-positive,
...
def run_generate_batch(
    ...
    *,
    ...
    :param max_length: Maximum tokenized length for inputs.
    :param torch_module: Imported :mod:`torch` module.
    :param stopping_criteria_list_cls: Class used to construct stopping
        criteria lists.
