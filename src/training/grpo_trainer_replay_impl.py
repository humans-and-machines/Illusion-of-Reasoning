...
from .runtime.env import GRPOTrainer, dist, torch
...
class GRPOTrainerReplay(GRPOTrainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if inject_every_batch:
            logger.warning(
                "inject_every_batch flag is currently unused; it is accepted for backward compatibility only.",
            )
    ...
    def _update_generation_temperature(self, step: int) -> float:
        ...
        if step < schedule.anneal_steps:  # linear decay
            ...
            new_temperature = schedule.start_temperature + frac * (
    ...
    def _maybe_mix_easy_batch(self, generation_batch, step: int) -> Any:
        ...
        if _is_rank0(self.accelerator):
            print(f"[MixDBG] step={step} bs={batch_size} p_easy={prob_easy:.2f}")

        mix_uniform = random.random()
        do_mix = (batch_size >= num_replicas) and (prob_easy > 0.0) and (mix_uniform < prob_easy)
        print("Are we mixing?", do_mix, prob_easy, mix_uniform)

        if not do_mix:
            if _is_rank0(self.accelerator):
                print(f"[Mix] step={step} skipped (bs={batch_size}, p_easy={prob_easy:.2f})")
            return generation_batch
        ...
        if _is_rank0(self.accelerator):
            print(f"[Mix] step={step} p_easy={prob_easy:.2f} → inserted 1 group of {num_replicas} at [0:4)")
        return generation_batch
    ...
    def _maybe_inject_replay_group(
        ...
        if not (inject_now and is_rank0 and len(self.replay_settings.buffer) >= self.replay_settings.warmup_steps):
            return generation_batch

        uid = group = None
        try:
            uid = self.replay_settings.buffer.sample_uid(mix_exploit_ratio=self.replay_settings.mix_exploit_ratio)
    ...
    def _maybe_credit_injected_uids(self, out: Dict[str, Any]) -> None:
        ...
        for uid in self.runtime_state.latest_injected_uids:
            try:
                ...
                if _is_rank0(self.accelerator):
                    print(f"[Replay][WARN] update_priority_by_uid failed (uid={uid}): {error}")
    ...
    def _compute_mean_reward_for_credit(self, out: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if _is_rank0(self.accelerator):
                print(
                    f"[Replay][WARN] couldn't aggregate rewards for crediting: {error}",
                )
        return mean_reward
    ...
    def _generate_and_score_completions(self, inputs):
        ...
        # rank / world info
        _is_init = getattr(dist, "is_initialized", lambda: False)
        _get_rank = getattr(dist, "get_rank", lambda: 0)
        _get_world = getattr(dist, "get_world_size", lambda: 1)
        dist_ok = _is_init() if callable(_is_init) else False
        rank = _get_rank() if dist_ok else 0
        world = _get_world() if dist_ok else 1
        is_rank0 = rank == 0
        ...
        completions_txt = self._decode_replay_completions(out, batch_info, tokenizer, is_rank0)
        if completions_txt is None:
            return out

        completions_txt, batch_info = self._align_replay_metadata(inputs, out, completions_txt, batch_info)
    ...
    def _maybe_throttle_vllm(self) -> None:
        if not self.accelerator.is_main_process or self.runtime_state.vllm_cooldown <= 0:
            return
        now = time.time()
    ...
    def _build_replay_batch_info(self, inputs: Any) -> tuple[dict[str, Any], Any]:
        if isinstance(inputs, list):
            clean_inputs = [
                {key: value for key, value in example.items() if not str(key).startswith("_")} for example in inputs
            ]
        ...
        injected = isinstance(inputs, list) and any("_buf_uid" in example for example in inputs)
        ...
        if isinstance(clean_inputs, list) and batch_size > 0:
            batch_info["gold_answers"] = [example.get("answer") for example in clean_inputs]
            batch_info["prompts_list"] = [example.get("prompt") for example in clean_inputs]
            batch_info["tasks_list"] = [example.get("task", "MATH") for example in clean_inputs]
            batch_info["boards_list"] = [example.get("board") or example.get("board_str") for example in clean_inputs]
            batch_info["sizes_list"] = [example.get("size") or example.get("N") for example in clean_inputs]
            batch_info["moves_list"] = [example.get("moves") or example.get("gold_moves") for example in clean_inputs]
        ...
        if (
            ...
            anchor_index = next(idx for idx, example in enumerate(inputs) if "_buf_uid" in example)
    ...
    def _decode_replay_completions(
        ...
        if completions_txt is None:
            if is_rank0:
                print(
                    "[Replay][DEBUG] no decodable completions; skipping replay push.",
                )
            return None
    ...
    def _decode_replay_from_ids(
        ...
        if total % batch_size != 0:
            if is_rank0:
                print(
                    f"[Replay][DEBUG] unexpected shapes: total={total}, batch_size={batch_size}",
                )
            return None
    ...
    def _decode_replay_from_text_keys(
        ...
        for key in TEXT_COMPLETION_KEYS:
            if key in out and isinstance(out[key], list) and out[key]:
                ...
                return [texts[index * num_candidates : (index + 1) * num_candidates] for index in range(batch_size)]
        return None
    ...
    def _align_replay_metadata(
        ...
        if isinstance(inputs, list) and any("_buf_uid" in example for example in inputs):
            anchor_index = next(idx for idx, example in enumerate(inputs) if "_buf_uid" in example)
    ...
    @staticmethod
    def _patch_out_for_injected(out: Dict[str, Any], batch_info: Dict[str, Any]) -> None:
    ...
    def _attach_rewards_from_completions(
        ...
        if isinstance(scores_nested, list) and scores_nested and isinstance(scores_nested[0], list):
    ...
    def _build_local_replay_winners(
        ...
        max_candidates = 4
        limited_completions = [completions[:max_candidates] for completions in completions_txt]
        ...
        for index in range(batch_size):
            ...
            accuracies = pure_accuracy_reward(preds_for_prompt, [gold] * len(preds_for_prompt))
            hit_index = next(
                (idx for idx, accuracy in enumerate(accuracies) if accuracy == 1.0),
                None,
            )
            ...
            if dist_info.get("is_rank0", False):
                print(
                    f"[Replay][HIT] rank={dist_info.get('rank', 0)} i={index} gold='{gold}'",
                )
    ...
    def _gather_replay_winners(
        ...
        if world_size > 1 and dist.is_initialized():
            ...
            winners_all = [example for group in gathered for example in (group or [])]
            if dist_info.get("is_rank0", False):
    ...
    def _push_replay_winners_to_buffer(
        ...
        if not (self.model.training and is_rank0 and self.replay_buffer is not None):
            return
        ...
        def _signature(example: dict[str, Any]) -> tuple[str, str]:
            ...
            if isinstance(messages_value, str):
                ...
                user_text = " ".join(
                    message.get("content", "").strip() for message in messages_value if message.get("role") == "user"
                )
        ...
        if success and int(uid) >= 0:
            ...
            debug_state = getattr(self.replay_buffer, "debug_state", lambda: {})()
            print(
    ...
    def _inject_group(self, generation_batch, uid: int, group: list[dict[str, Any]]):
        ...
        if _is_rank0(self.accelerator):
            ...
            def _head_user(batch_example):
                try:
                    msgs = batch_example.get("messages") or batch_example.get("prompt") or []
                    if isinstance(msgs, list):
                        return next(
                            (message.get("content", "") for message in msgs if message.get("role") == "user"),
                            "<no-user>",
                        )
                    return str(msgs)[:60]
