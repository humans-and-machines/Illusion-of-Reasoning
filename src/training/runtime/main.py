...
from .env import datasets, get_last_checkpoint, get_peft_config, set_seed, transformers


try:
    from ..utils import get_dataset, get_model, get_tokenizer
except ImportError:  # pragma: no cover - fallback for environments without training deps
    # Define placeholders so callers/tests can monkeypatch these attributes even if
    # the real dependencies (torch/transformers) are not installed in the runtime.
    def get_dataset(*_args, **_kwargs):
        """Stub when training utilities are unavailable."""
        raise RuntimeError("get_dataset requires training dependencies (torch/transformers)")

    def get_model(*_args, **_kwargs):
        """Stub when training utilities are unavailable."""
        raise RuntimeError("get_model requires training dependencies (torch/transformers)")

    def get_tokenizer(*_args, **_kwargs):
        """Stub when training utilities are unavailable."""
        raise RuntimeError("get_tokenizer requires training dependencies (torch/transformers)")


from ..grpo_dataset import _load_easy_pool, _make_conversation
...
def _configure_logging(training_args: Any) -> None:
    ...
    if datasets is not None and getattr(datasets, "utils", None) is not None:
        datasets.utils.logging.set_verbosity(log_level)
    if transformers is not None and getattr(transformers, "utils", None) is not None:
...
def _build_reward_funcs(
    ...
    if isinstance(reward_funcs, dict):
        return {key: _wrap_reward_for_nested(fn) for key, fn in dict(reward_funcs).items()}
