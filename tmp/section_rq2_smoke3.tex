...
In \emph{Xwords} and \emph{Math} shifts are uncommon ($\%S=1.058$; $\%S=4.225$) and are mildly harmful ($\mathrm{AME}=0.0000$, $p=0$; $\mathrm{AME}=0.0000$, $p=0$).

In \emph{RHour}, shifts are comparatively frequent ($\%S=42.300$) but have no measurable effect on accuracy ($\mathrm{AME}\approx0.0000$, $p=0$).
...
On \emph{Xwords}, shifts are associated with a small but statistically significant positive marginal effect ($\mathrm{AME}=-0.0570$, $p=0.05916$), consistent with the raw contrast $\Delta=-2.45$pp.
On \emph{Math}, shifts are strongly harmful ($\mathrm{AME}=-0.0578$, $p=0.08759$; $\Delta=-17.13$pp).
On \emph{RHour}, shifts are frequent ($\%S=43.78$) but correctness is extremely low overall; accordingly, the estimated effect is statistically detectable yet numerically negligible ($\mathrm{AME}\approx-0.0010$, $p=4.15\times10^{-2826}$; $\Delta\approx-0.03$pp).
...
\input{tmp/table_rq2_rs_smoke3.tex}
