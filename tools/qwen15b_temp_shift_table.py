...
# -*- coding: utf-8 -*-
"""
Per-temperature shift metrics for Qwen2.5-1.5B (Xword / Math / Rush Hour).

For each decoding temperature T ∈ {0, 0.05, 0.3, 0.7} and domain, this script
computes:

  - N                : number of test samples (steps in [min_step, max_step])
  - %S               : shift prevalence (100 × share of samples with GPT shift)
  - p_Y|S=1          : P(correct | shift = 1)
  - Δ%               : (p_Y|S=1 − p_Y|S=0) in percentage points
  - AME              : average marginal effect of shift in a Binomial GLM
  - p                : GLM p-value for the shift coefficient

Usage (from repo root, canonical GPT shifts, no words gate, steps 0–950):

  python tools/qwen15b_temp_shift_table.py

You can also override basic settings, e.g.:

  python tools/qwen15b_temp_shift_table.py --min_step 0 --max_step 950 \\
      --gpt_mode canonical --no_gate_by_words
"""
...
# this script is invoked as ``python tools/qwen15b_temp_shift_table.py``.
...
# Roots per temperature for Qwen2.5-1.5B (GRPO) runs
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Compute per-temperature shift metrics (N, %%S, p_Y|S=1, Δ%%, AME, p) "
...
def _load_rows_for_temp_domain(
    temp_value: float,
    domain_key: str,
    root: str,
    split_filter: str,
    ...
    """
    Load (correct, shift, problem) rows for a (temp, domain) pair.
    """
...
def main() -> None:
    """
    Compute and print per-temperature metrics suitable for LaTeX tables.
    """
    parser = build_argparser()
    args = parser.parse_args()

    # Try to obtain statsmodels; fall back gracefully if unavailable.
    try:
    ...
    for temp_value, dom_roots in sorted(ROOTS_BY_TEMP.items()):
        for domain_key, root in dom_roots.items():
            ...
            n_noshift = N - n_shift
            share_shift = (n_shift / N) * 100.0 if N else np.nan
            ...
            dom_label = DOMAIN_LABELS.get(domain_key, domain_key)
            print(
                f"{temp_value:.2g},{dom_label},{N},"
                f"{share_shift:.3f},{p_shift:.4f},{delta_pp:.2f},{ame:.4f},{p_glm:.3g}",
            )
